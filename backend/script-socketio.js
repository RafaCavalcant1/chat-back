...
const io = new Server(server, {
  cors: {
    origin: ["http://127.0.0.1:5500", "null"], // Inclui null como origem permitida
    methods: ["GET", "POST"],
  },
});
...
app.get("/cat-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "gato.mp3");
  res.sendFile(audioPath);
});

//som do scooby
app.get("/scooby-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "scooby.mp3");
  res.sendFile(audioPath);
});

//som do cavalo
app.get("/cavalo-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "cavalo.mp3");
  res.sendFile(audioPath);
});

//som do cachorro
app.get("/dog-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "cachorro.mp3");
  res.sendFile(audioPath);
});

//som da galinha
app.get("/galinha-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "galinha.mp3");
  res.sendFile(audioPath);
});

//som do spiderman
app.get("/spiderman-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "spiderman.mp3");
  res.sendFile(audioPath);
});

//som do sapo
app.get("/sapo-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "sapo.mp3");
  res.sendFile(audioPath);
});

//som do ari
app.get("/ari-sound", (req, res) => {
  const audioPath = path.join(__dirname, "..", "audios", "ari.mp3"); // Sobe um nível para acessar "audios"
  res.sendFile(audioPath);
});
